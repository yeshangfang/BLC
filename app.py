...
def draw_dna_structure(level):
    ...
    # 随机选择甲基化的位置 (仅在启动子区域)
    # 为了演示稳定，这里简单取前 N 个，实际可改为随机 sample
...
with col_left:
    ...
    filter_style = f"filter: brightness({brightness}) grayscale({grayscale}); border-radius: 15px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);"

    try:
        # 尝试加载你提供的黄色小鼠图片
        st.image("yellow.png", caption="Agouti 小鼠表型", use_column_width=True)

        # 使用 HTML/CSS 叠加滤镜层 (Streamlit原生image不支持直接传style，需用HTML包裹)
        # 这里为了简单兼容，直接使用 markdown + HTML img 标签来实现滤镜
        st.markdown(f"""
